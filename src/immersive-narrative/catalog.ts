...
export const NARRATIVE_ARCHETYPES: NarrativeArchetype[] = [
  {
    ...
    moments: (context) => [
      ...
      },
      {
        ...
        sceneRole: "threshold",
        whatHappens: `${context.pronoun.subject} reaches the ${scene(context, "threshold")} and settles beside the closed door.`,
        causalLink: "The approach ends at the same closed threshold reached from the hallway.",
      },
      {
        purpose: "micro_event",
        sceneRole: "threshold",
        whatHappens: `Beside the door, ${context.pronoun.subjectLower} touches the bag, misses the key on the first touch, immediately finds the key with a second touch, and unlocks the door.`,
        causalLink: "The brief second touch follows the first touch missing the key.",
      },
      {
        purpose: "response",
        sceneRole: "threshold",
    ...
    alternateMoments: [
      (context) => [
        ...
        },
        {
          purpose: "approach_trigger",
          sceneRole: "threshold",
          whatHappens: `At the ${scene(context, "threshold")}, ${context.pronoun.subjectLower} settles beside the closed door.`,
          causalLink: "The approach ends at the same closed threshold reached from the hallway.",
        },
        {
          purpose: "micro_event",
  ...
  },
  {
    ...
    storyIntent: "Capture a quiet cafe arrival as the character notices an available seat and naturally adjusts their route toward it.",
    initialCharacterState: (context) => {
      const { pronoun } = context;
      return `${pronoun.subject} steps in from the street at a steady pace. ${pronoun.possessive} attention moves from the doorway into the cafe, and the outside pace is still present in the first few steps. ${context.toneClause} ${context.seasonClause}`;
    },
    microEvent: (context) => `After noticing an open seat in the cafe, ${context.pronoun.subject} adjusts the route through the clear aisle toward it.`,
    emotionalArc: ["moving", "noticing", "adjusting route", "settling"],
    moments: (context) => [
      {
        purpose: "establish_state",
        sceneRole: "interior",
        whatHappens: `${context.pronoun.subject} enters the ${scene(context, "interior")} and walks through its entry area into the room without stopping at the doorway.`,
        causalLink: null,
      },
      {
        purpose: "approach_trigger",
        sceneRole: "counter",
        whatHappens: `${context.pronoun.subject} continues past the counter as ${context.pronoun.possessiveLower} gaze naturally registers an open seat across the cafe; ${context.pronoun.possessiveLower} stride does not break.`,
        causalLink: "The open seat becomes visible as the character moves farther into the cafe, giving them an immediate destination within the visit.",
      },
      {
        purpose: "micro_event",
        sceneRole: "counter",
        whatHappens: `${context.pronoun.subject} turns ${context.pronoun.possessiveLower} shoulders slightly toward the open seat and carries the next step into the clear aisle without breaking stride.`,
        causalLink: "The small route adjustment follows directly from the open seat becoming visible across the cafe.",
      },
      {
        purpose: "response",
        sceneRole: "interior",
        whatHappens: `${context.pronoun.subject} slows as the open chair draws near, shortening ${context.pronoun.possessiveLower} steps and easing into the seating area.`,
        causalLink: "The pace eases because the chosen seat has come within reach at the end of the adjusted route.",
      },
      {
        purpose: "after_state",
        sceneRole: "interior",
        whatHappens: `${context.pronoun.subject} turns into the open chair and takes a seat, then settles into the next part of ${context.pronoun.possessiveLower} cafe visit.`,
        causalLink: "Reaching the seat completes the short arrival route and lets the cafe visit continue from a settled position.",
      },
    ],
    alternateMoments: [
      (context) => [
