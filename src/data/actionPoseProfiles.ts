...
const sceneActionLines: Partial<Record<Exclude<TeamScenePreference, "自动匹配">, string>> = {
  ...
  酒店咖啡厅内: "Use a natural walk toward a seat, a brief table-side pause, or a relaxed seated moment with both feet grounded; keep tables, chairs, guests, and any coffee cup clear of the sneakers.",
  ...
  if (input.scenePreference === "旅行酒店") {
    return pick(
      [
        ...
        "Finish a short lobby-to-room transition step and pause naturally, with relaxed shoulders and stable shoe-floor contact."
      ],
      input,
      67
    );
