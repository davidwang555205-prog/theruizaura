...
const multiImageExpressionSequenceLines = [
  "Expression beat for this image: a brief friendly response to the nearby camera, with a faint asymmetric smile and focused alive eyes that visibly differ from the other images — the eye direction and catchlight pattern must feel unique to this body angle. If facing the camera directly, both eyes carry balanced focus; if in a three-quarter or side view, the eyes shift naturally toward the lens while the face keeps its original angle, avoiding a stiff head-on stare or cloned gaze from the previous frame.",
...
const nonProductAtmosphereNegativeLine =
  "Avoid full person, model, posed body, influencer styling, fake luxury display, visible brand logos, sterile CGI interior, excessive props, fake brand text, AI gibberish, and anything disconnected from THERUIZ AURA's quiet warm luxury world.";
